...
HTMLCODE;
		...
		function transcodeStream($stream, $required=0, $detect=2, $convert=1)
		{
			...
			if ($detect == 2 || ($detect == 1 && !$videoCodec))
			{
				...
				$cmd = $options['ffmpegPath'] .' -y -i "' . $rtmpAddressViewI .'/'. $stream . '" 2>&1 ';
				$info = shell_exec($cmd);
...
<h2 class="nav-tab-wrapper">
   ...
   <a href="admin.php?page=live-streaming&tab=watch-params" class="nav-tab <?php echo $active_tab=='watch-params'?'nav-tab-active':'';?>">Watch Params</a>
    ...
    <a href="admin.php?page=live-streaming&tab=hls" class="nav-tab <?php echo $active_tab=='hls'?'nav-tab-active':'';?>">Mobile</a>
...
<?php
			...
			case 'hls':
?>
<h3>Mobile HTML5 HLS & Transcoding</h3>
Configure transcoding and HTML5 HLS delivery to mobile devices and Safari.
...
<input name="rtmp_restrict_ip" type="text" id="rtmp_restrict_ip" size="20" maxlength="48" value="<?php echo $options['rtmp_restrict_ip']?>"/>
<BR>Allow status updates only from configured IP. If not defined will configure automatically when first successful webLogin authorisation occurs for a broadcaster. Web status will not work if this is empty or not configured right.
<!--
	...
	<?php
	 ...
	 <?php
		...
		function streamSnapshot($stream, $ipcam = false)
		{
			...
			$cmd = $options['ffmpegPath'] . " -f image2 -vframes 1 \"$filename\" -y -i \"" . $rtmpAddressView ."/". $stream . "\" >&$log_file & ";
