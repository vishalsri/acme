	...
	<style>
		...
		.btn_dul.hvr-shutter-out-horizontal:before {
			...
			.card-block-info::before {
				content: "";
				display: block;
				background: url(<?php echo $theme_options['ship_icon']['url'];?>) no-repeat;
				position: absolute;
				left: 0;
				right: 0;
				height: 50px;
...
<body <?php body_class(); ?>>
	...
	<div class="container-fluid">
		...
		<div class="row pos_rel">
			<?php
				$no_slides = $theme_options['header-slides'];
				$counter =count($no_slides);
				if($counter == 1){
				//echo '<pre>';print_r(count($sliders));echo '</pre>';die('here');
			?>
			<div class="gallery js-flickity" data-flickity id="carousel" data-flickity-options='{ "prevNextButtons": false,"pageDots":false }'>
			<?php }else{ ?>
			<div class="gallery js-flickity" data-flickity id="carousel" data-flickity-options='{ "prevNextButtons": false }'>
			<?php } ?>
				<?php
					foreach($theme_options['header-slides'] as $slide) {
				?>
				<div class="gallery-cell"><img src="<?php echo $slide['image']; ?>" alt="slider"></div>
				<?php } ?>
			</div>
